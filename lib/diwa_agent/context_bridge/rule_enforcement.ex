  ...
  defp get_active_rules(context_id) do
    ...
    query =
      from(m in MemorySchema,
        ...
        where:
          m.memory_class in ["user_rule", "system_instruction"] or
            fragment("json_extract(?, '$.type') = ?", m.metadata, "rule")
      )
      ...
      case Regex.compile(pattern) do
        {:ok, re} ->
          if String.match?(content, re) do
            ...
            %{
              rule_id: rule.id,
              title: meta["title"] || rule.content,
              message: meta["violation_message"] || "Rule not followed"
            }
          end
