defmodule DiwaAgent.Session.Process do
    ...
    action =
      case reason do
        ...
        r when r in [:normal, :shutdown] ->
          Logger.info(
            "[SessionProcess] Graceful shutdown without auto-commit for session #{session_id}"
          )
        ...
        {:error, reason_detail} ->
          Logger.error(
            "[SessionProcess] Session #{session_id} crashed: #{inspect(reason_detail)} - marking orphaned"
          )
        ...
        other ->
          Logger.warning(
            "[SessionProcess] Session #{session_id} terminated unexpectedly: #{inspect(other)} - marking orphaned"
          )
  ...
  defp handle_graceful_shutdown(state) do
    ...
    if buffer_count > 0 do
      Logger.info(
        "[SessionProcess] Auto-committing #{buffer_count} operations for session #{session_id}"
      )
