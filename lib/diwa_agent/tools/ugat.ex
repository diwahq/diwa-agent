defmodule DiwaAgent.Tools.Ugat do
  ...
  def execute("detect_context", %{"type" => type, "value" => value}) do
    case Ugat.detect_context(type, value) do
      ...
      nil -> error_response("No context found matching #{type} = '#{value}'")
    end
  end
      ...
      {:ok, _} ->
        ...
        case Ugat.add_binding(cid, type, value, metadata) do
          {:ok, binding} -> 
             ...
             Binding: #{type} -> #{value}
             """)
          {:error, changeset} -> error_response("Binding failed: #{inspect(changeset.errors)}")
        end
      {:error, :not_found} -> error_response("Context not found: #{cid}")
    end
  end
  ...
  def execute("link_contexts", %{"source_context_id" => src, "target_context_id" => tgt, "relationship_type" => type} = args) do
    ...
    with {:ok, _} <- Context.get(src),
       ...
       case Ugat.link_contexts(src, tgt, type, metadata) do
         {:ok, link} ->
            ...
            ID: #{link.id}
            """)
         {:error, :circular_dependency_detected} -> error_response("Failed: Circular dependency detected in 'depends_on' chain.")
         {:error, :self_reference} -> error_response("Failed: Cannot link a context to itself.")
         {:error, changeset} -> error_response("Linking failed: #{inspect(changeset.errors)}")
       end
    else
  ...
  def execute("get_related_contexts", %{"context_id" => cid} = args) do
    ...
    if links == [] do
      ...
      list = Enum.map(links, fn r ->
        target_name = if Ecto.assoc_loaded?(r.target_context), do: r.target_context.name, else: "ID: #{r.target_context_id}"
        source_name = if Ecto.assoc_loaded?(r.source_context), do: r.source_context.name, else: "ID: #{r.source_context_id}"
        
        rel_str = case direction do
          :outgoing -> "-> #{target_name} [#{r.relationship_type}]"
          :incoming -> "<- #{source_name} [#{r.relationship_type}]"
          :both -> 
  ...
  def execute("get_context_graph", %{"root_id" => root_id} = args) do
    ...
    case Ugat.get_context_graph(root_id, opts) do
      ...
      {:error, reason} -> error_response("Graph traversal failed: #{inspect(reason)}")
    end
  end
  ...
  def execute("get_dependency_chain", %{"context_id" => cid}) do
    case Ugat.get_dependency_chain(cid) do
      ...
      {:error, :cycle_detected} -> error_response("Failed: Cycle detected in dependencies.")
      {:error, reason} -> error_response("Failed to retrieve chain: #{inspect(reason)}")
    end
  end
    ...
    cond do
      ...
      true ->
        case Ugat.navigate(cid, target_path, mode) do
            ...
            body = case result.mode do
              "list" -> 
                if result.data == [] do
                  ...
                  items = Enum.map(result.data, fn n -> 
                    dir_icon = if n.direction == :outgoing, do: "hero-arrow-up-right", else: "hero-arrow-down-left"
                    dir_color = if n.direction == :outgoing, do: "text-emerald-400", else: "text-amber-400"
                    rel_badge = case n.relationship do
                ...
                <div class="bg-indigo-500/5 border border-indigo-500/20 rounded-2xl p-6 shadow-2xl relative overflow-hidden group my-4">
                  ...
                  <div class="relative z-10">
                    ...
                    <div class="mt-6 flex flex-wrap gap-2">
                      #{if d.bindings == [], do: "<span class='text-[10px] italic opacity-40 italic'>No bindings</span>", else: 
                        Enum.map(d.bindings, fn b -> 
                          "<span class='px-2 py-1 rounded-md bg-white/5 border border-white/10 text-[9px] font-mono text-slate-400'>#{b}</span>"
                        end) |> Enum.join("")
                      }
                    </div>
                  </div>
          ...
          {:error, :no_parent_found} -> error_response("Cannot go up '..': No parent context found.")
          {:error, :path_not_found} -> error_response("Path not found: '#{target_path}'")
          {:error, reason} -> error_response("Navigation failed: #{inspect(reason)}")
        end
    end
  end
      ...
      if context_id do
         ...
         case DiwaAgent.ContextBridge.LiveSync.start_session(context_id, actor, %{}) do
           {:ok, session} ->
             ...
             instructions = DiwaAgent.Tools.ClientInstructions.get_instructions(client_type: client_type)
  ...
  def execute("confirm_binding", args) do
    ...
    binding_value = Map.get(args, "binding_value") || 
      (if binding_type == "git_remote", do: detect_git_remote(), else: Map.get(args, "path"))

    if is_nil(binding_value) do
       error_response("Could not determine binding value (path/remote). Please provide explicitly.")
    else
      # 2. Resolve Context ID
      context_id = Map.get(args, "context_id") || 
        (if name = args["context_name"] do
           case DiwaAgent.Storage.Context.find_by_name(name) do
             {:ok, ctx} -> ctx.id
             _ -> nil
           end
         end)
      ...
      case {action, context_id} do
        ...
        _ -> error_response("Invalid action or missing context: #{action}")
      end
    end
  end
  ...
  defp resolve_context_id(args) do
    ...
    cond do
      Map.has_key?(args, "context_id") -> Map.get(args, "context_id")
