defmodule DiwaAgent.Storage.Context.Ugat do
  ...
  def detect_context(type, value) do
    query = from b in ContextBinding, 
  ...
  defp derive_match_reason(score, _keywords, _git_remote, _ctx) when score >= 0.7, do: "High-confidence match (Name & Org)"
  defp derive_match_reason(score, _keywords, _git_remote, _ctx) when score >= 0.4, do: "Moderate match (Name similarity)"
  ...
  def get_dependency_chain(context_id) do
    case DiwaAgent.Storage.Context.get(context_id) do
      {:ok, _ctx} ->
        ...
        ordered_contexts = 
          sorted_ids
          |> Enum.reverse() # topo_visit builds reverse post-order (Root, Child...) -> Reverse to get (Child, Root)
  ...
  defp topo_visit(id, visited, acc) do
    if MapSet.member?(visited, id) do
      ...
      {final_acc, final_visited} = Enum.reduce(deps, {acc, new_visited}, fn dep_id, {a, v} -> 
  ...
  defp traverse(current_ids, depth, state) do
    {next_ids, new_state} = Enum.reduce(current_ids, {[], state}, fn id, {acc_next, acc_state} ->
      ...
      Enum.reduce(relationships, {acc_next, acc_state}, fn rel, {curr_next_ids, curr_state} ->
        ...
        # Update Edge
        edge = %{source: rel.source_context_id, target: rel.target_context_id, type: rel.relationship_type}
        updated_edges = MapSet.put(curr_state.edges, edge)
        
        # Update Node
        updated_nodes = Map.put(curr_state.nodes, neighbor_id, %{id: neighbor_id, name: neighbor_context.name})
        ...
        if MapSet.member?(curr_state.visited, neighbor_id) do
          ...
          {[neighbor_id | curr_next_ids], %{curr_state | edges: updated_edges, nodes: updated_nodes, visited: updated_visited}}
        end
      end)
    end)
  ...
  defp resolve_path(current_id, "..") do
    ...
    query = from r in ContextRelationship,
      where: (r.source_context_id == ^current_id and r.relationship_type == "child_of") or
             (r.target_context_id == ^current_id and r.relationship_type == "contains"),
      limit: 1
      
    case Repo.one(query) do
      nil -> {:error, :no_parent_found}
      %{source_context_id: _src, target_context_id: tgt, relationship_type: "child_of"} -> {:ok, tgt}
      %{source_context_id: src, target_context_id: _tgt, relationship_type: "contains"} -> {:ok, src}
    end
  end
  ...
  defp resolve_path(current_id, target_name) do
    ...
    case Ecto.UUID.cast(target_name) do
      {:ok, uuid} -> 
        # Verify it exists
        if Repo.get(DiwaSchema.Core.Context, uuid), do: {:ok, uuid}, else: {:error, :context_not_found}
  ...
  def analyze_impact(context_id) do
    ...
    |> MapSet.delete(context_id) # Don't include self
    |> MapSet.to_list()
      ...
      {{:value, {current_id, path}}, next_queue} ->
        if current_id == target do
          ...
          if MapSet.member?(visited, current_id) do
            ...
            new_queue = Enum.reduce(neighbors, next_queue, fn neighbor, q ->
