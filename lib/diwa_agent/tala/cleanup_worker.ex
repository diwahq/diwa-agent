defmodule DiwaAgent.Tala.CleanupWorker do
  ...
  def init(_opts) do
    if cleanup_enabled?() do
      Logger.info(
        "[TALA Cleanup] Worker started (interval: #{cleanup_interval_ms()}ms, TTL: #{buffer_ttl_hours()}h)"
      )
  ...
  def handle_info(:cleanup, state) do
    ...
    Logger.info(
      "[TALA Cleanup] Complete: #{result.buffers_cleaned} buffers cleaned, #{result.warnings_created} incidents created"
    )
  ...
  defp perform_cleanup do
    ...
    results =
      Enum.map(orphaned_sessions, fn session_id ->
  ...
  defp create_cleanup_incident(session_id, op_count) do
    ...
    if context_id do
      ...
      case DiwaAgent.Storage.Memory.add(context_id, content, %{
        ...
        {:ok, _memory} ->
          Logger.warning(
            "[TALA Cleanup] Created incident for session #{session_id} (#{op_count} ops)"
          )
      ...
      Logger.warning(
        "[TALA Cleanup] Could not create incident for session #{session_id} - no context found"
      )
