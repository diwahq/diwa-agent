defmodule DiwaAgent.Tools.Ugat.LinkingTest do
  ...
  describe "MCP Tool Wrappers" do
    ...
    test "detect_context via execute/2", %{c1: c1} do
      {:ok, _} = DiwaAgent.Storage.Context.Ugat.add_binding(c1.id, "git_remote", "https://git.com/alpha")
      
      response = Ugat.execute("detect_context", %{"type" => "git_remote", "value" => "https://git.com/alpha"})
    ...
    test "bind_context via execute/2", %{c1: c1} do
      response = Ugat.execute("bind_context", %{
        "context_id" => c1.id,
        "type" => "path",
        "value" => "/tmp/project"
      })
    ...
    test "link_contexts via execute/2", %{c1: c1, c2: c2} do
      response = Ugat.execute("link_contexts", %{
